...
where $\sigma$ corresponts to a sigmoid function. In other words, Function \eqref{eq:cinque} is a Neural Network composed of one single layer of 3 neurons. The activation functions are sigmoid, and the task is a classification since the output is binary. \\
Function \eqref{eq:cinque} can be written as a ***graph*** where the intermediate variables are explicitly specified, as shown in Figure 1.
