...
tags: [Bayesian ]     # TAG names should always be lowercase
comments: true
toc: true
math: true
---

The goal of this tutorial is to share a probabilistic approach for quantifing the uncertainty around the number of shots made by footbal teams given the characteristic of the match being played. 
...
Specifically, we have a collection of historical matches of the season 2023/2024 played until the date bla/bla/bla considering each team of the TOP 5 leagues in Europe (ENG, ITA, FRA, ESP, GER). For each match, the following features are gathered:
...
*Venue*
: If 1, the match has been played in the *Team*'s home, conversely the *Team* is playing away,

*Time*
: The index based on the sequential order with which the matches have being played. 

> The dataset has been formed by joining togheter `FbRef` data and `kaggle`'s dataset. 
{: .prompt-info }
...
- When the match is played in the *Team*'s home, we expecet to observe a higher number of shots. 
- Higher is the difference between *Att_rating* and *Def_rating*, higher is the number of shots observed. 
- Matches which has been played closed togheter with respect to the *Time* might be more correlated between each other.

## Data Visualization

Let's find out if the data seems to support the formulated hypotheses.

### Mean(shots) distribution
We can see how the mean shots varies among the teams. Moreover, by segmenting by *Venue*, it can be noticed that the mean shots is higher when the match has been played in the *Team*'s home, as expected. 
```R
df %>%
  group_by(Team, Venue) %>%
  summarise(
  ...
  labs(x = "Mean(shots)")
```
![Desktop View](first.png){: width="972" height="589"}
...
df %>%
  ...
  labs(x = "Sd(shots)")
```
![Desktop View](second.png){: width="972" height="589"}
...
```
![Desktop View](third.png){: width="972" height="589"}
