...
##  Introduction
The following is a brief snapshot of the dataset used in this case study,

| Team           | Opponent       | Shots          | Venue          |
|:--------------:|:--------------:|:--------------:|:--------------:|
| arsenal        | forest         | 15             | 1              |
| forest         | arsenal        | 6              | 0              |
| ...            | ...            | ...            | ...            |
| berlin         | leverkusen     | 2              | 0              |
| berlin         | augsburg       | 14             | 1              |

As we can see, we have a collection of historical matches of football teams from the top leagues in ENG, ITA, FRA, ESP and GER of the season 2023/2024. More specifically:
...
As we can spot, assuming that the rows of the dataset are indipendent between each other might be limiting since some rows refers to exactly the same match. For example, the number of shots made by Arsenal can impact the number of shots made by Nottingham Forest, first of all for the nature of the game since

 because the time is limited in football, secondly 


modeling directly the random variable *Shots* might be limited  assuming that each row is indipendent between each other might represent a wrong assumption since 
