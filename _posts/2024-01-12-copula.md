...
Let's imagine the scenario in which we have two random varialbes $$Y_home$$ and $$Y_away$$.
The main motivation for using a Copula model for modelling football teams' shots is that in each match there are two teams facing each other. Therefore a Copula model gives us the flexibility to 
