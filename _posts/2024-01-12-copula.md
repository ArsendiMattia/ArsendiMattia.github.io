...
As we can spot, directly modeling the number of shots of each team implies assuming that the observations are independent. In other words, the shots made by Arsenal against Nottingham Forest in the game played on 2023-08-12 are independent of those made by Nottingham Forest in the same match and from the shots made by, for example, Union Berlin on 2023-11-25. If the latter case is reasonable since they refer to two different matches, assuming that the shots made by teams in the same game are independent might be an inappropriate assumption. Conversely, we can expect that shots made by one team have a substantial impact on the number of shots made by the opponent for different reasons, which might span, for example, from more apparent explanations, such as if one team collects many shots, the opponent has less time to organize its offensive maneuver, to more complex ones such as teams which control the game do not concede any space to the opponents.
