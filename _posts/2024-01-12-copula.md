...
##  Introduction
Let's take an initial look at how the dataset is composed.
...
Directly modeling the shots made by each team implies assuming that the observations are independent. Namely, the shots made by Arsenal against Nottingham Forest in the game played on 2023-08-12 are:

- independent of those made by Nottingham Forest in the same match,
- independent of those made, for example, by Union Berlin on 2023-11-25.

If the latter case is reasonable since they refer to two different matches, assuming that the shots made by teams in the same game are independent might be an inappropriate assumption. \\
Conversely, we expect that shots made by one team substantially impact the number of shots made by the opponent. For example, if one team collects many shots, the opponent has less time to organize its offensive maneuver, thus causing it to collect fewer shots. 

Copula models grant us a flexible approach to modeling the dependency between the shots made by the home and away teams. Indeed, the key feature of a Copula approach is that the marginal distribution of each variable is built separately; only then is the Copula model introduced on top of the marginals to model the joint distribution, thus allowing an elegant model development strategy. \\
Therefore, the previously shown dataset observations are grouped by *Team* and *Date*, leading to the following dataset in which we can reasonably assume that the observations are independent.

| Team           | Opponent       | Shots Team     | Shots Opponent |
|:--------------:|:--------------:|:--------------:|:--------------:|
| arsenal        | forest         | 15             | 6              |
| chelsea        | liverpool      | 10             | 13             |
| ...            | ...            | ...            | ...            |
| leverkusen     | berlin         | 18             | 2              | 
| berlin         | augsburg       | 14             | 11             | 

Before diving into the Model Development step, let's explore the data to see if what we discuss actually makes sense and raise questions that we currently do not have.

## Data Exploration

First of all, let's see how the difference between Shots Team and Shots opponent behaves

![Desktop View](/posts/copula/first.png){: width="972" height="589"}
