...
If the latter case is reasonable since they refer to two different matches, assuming that the shots made by teams in the same game are independent might be an inappropriate assumption. Conversely, we expect that shots made by one team substantially impact the number of shots made by the opponent. For example, if one team collects many shots, the opponent has less time to organize its offensive maneuver, thus causing it to collect fewer shots. 

<!-- Copula models grant us a flexible approach to model the joint distribution between the shots made by the home team and the away team. In other words,  the Copula model introduced on top of the marginals to model the dependency between the variables.
 whichever quantitative variables we are interested in, which are two discrete variables in this specific case study. Specifically, the key feature of a Copula approach is that the marginal distribution of each variable is built separately; only then is the Copula model introduced on top of the marginals to model the dependency between the variables. -->
